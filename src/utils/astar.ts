// Port of javascript-astar 0.4.1 to typescript.
// Had to swap x and y coordinates as input wasn't intuitive (previously y was horizontal x was vertical)
...
export class Graph {
  ...
  constructor(gridIn: number[][], options = { diagonal: false }) {
    this.nodes = [];
    this.diagonal = !!options.diagonal;
    this.grid = [];

    // This does inverse of what I want
    // for (let x = 0; x < gridIn.length; x++) {
    //   this.grid[x] = [];

    //   for (let y = 0, row = gridIn[x]; y < row.length; y++) {
    //     console.log(`Adding ${x}, ${y} to grid`);
...
export const astar = {
  ...
  */
  search: function (
    ...
    options = { heuristic: astar.heuristics.manhattan, closest: false },
  ): GridNode[] {
    graph.cleanDirty();
    // options = options || {};
  ...
  cleanNode: function (node: GridNode): void {
    node.f = 0;
    node.g = 0;
    node.h = 0;
    node.visited = false;
